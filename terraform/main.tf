...
resource "yandex_vpc_security_group" "main" {
  ...
  ingress {
    ...
    protocol       = "TCP"
    port           = 80
    v4_cidr_blocks = var.allowed_ingress_cidr
  }
...
resource "yandex_compute_instance" "main" {
  ...
  network_interface {
    ...
    security_group_ids = var.enable_security_group ? [yandex_vpc_security_group.main[0].id] : []
  }
...
# Run: terraform import github_repository.course_repo[0] DevOps-Core-Course
resource "github_repository" "course_repo" {
  # Bonus resource must stay isolated from the main YC VM scenario.
  # Enable explicitly with: -var='enable_github_bonus=true'
  count = var.enable_github_bonus ? 1 : 0

  lifecycle {
    # Prevent accidental repo deletion if GitHub token is removed from local vars.
